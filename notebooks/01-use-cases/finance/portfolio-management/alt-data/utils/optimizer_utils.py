...
class Optimizer:
    ...
    def esg_min_volatility(self, df_price, esg_df, min_esg_score=0.5):
        ef = self.init_model(df_price)
        esg_scores = esg_df.values.flatten().tolist()

        # https://pyportfolioopt.readthedocs.io/en/latest/FAQ.html
        # esg_scores = [round(random.uniform(0.1, 0.9), 2) for x in range(0,df_price.shape[1])]
        # portfolio_min_score = 0.5
        # here, for cvxpy, the @ symbol does a dot product
        ef.add_constraint(lambda w: esg_scores @ w >= min_esg_score)
