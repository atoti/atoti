...
def monthly_rebal(df_portfolio, date, cq_alphaflow_daily):
    ...
    rebalanced = cq_alphaflow_daily[cq_alphaflow_daily['symbol'].isin(drop_zeros['symbol'].to_list())].copy(deep=True)
    
    # hold on to the zero-valued weights
    only_zeros = df_portfolio.drop(df_portfolio[df_portfolio["weight.SUM"] != 0].index).drop(columns=["weight.SUM"])
    only_zeros["weight"] = 0
    ...
    rebalanced["method5_ratio"] = rebalanced["method5_inst_buy"]/rebalanced["method5_inst_sell"]
    ...
    return pd.concat([rebalanced[["portfolio", "iteration", "method", "symbol", "weight"]], only_zeros])
